...
export default function usePin(compRef, initialState, pinLength, maxTries, onSuccess = () => {}, onFailure = () => {}) {
  ...
  const setPin = (newPin) => {
    if (!state.previousPin) {
      setState(prevState => ({ ...prevState, inputPin: '', previousPin: newPin, message: 'Confirme el PIN previo' }));
    } else if (newPin === state.previousPin) {
      toggleDisplay();
      dispatch({ type: SET_PIN, payload: newPin });
      onSuccess();
    } else {
      setState(prevState => ({ ...prevState, previousPin: '', inputPin: '',
  ...
  const checkPin = (currentPin) => {
    ...
    setState(prevState => ({ ...prevState, currentTries: state.currentTries + 1 }));
  };
